...
count(x -> seglength(x) / 48000 < 0.3, allesegs)

sl = map(seglength, allesegs) ./ 48000
ec = ecdf(sl)
plot(x -> ec(x), minimum(sl), maximum(sl))
ec(5)
slm = mean(sl)
inds = sortperm(sl, by=(x -> (x-slm)^2))
i = 1
begin
    i += 1
    plot(allesegs[1])
...
aantalplots = 0
for yi = 1:(size(x)[2])
    ys = collect(Iterators.partition(x[:,yi], 480000))
...
wavplay(x, samplerate)


    
ys = collect(Iterators.partition(x[:,2], 960000))
map(length, ys)
y = collect(ys[1])
plot(y)
...
length(y[segs[seg][1]:segs[seg][2]])
